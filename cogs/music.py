                ...
                if not player.is_paused() and player.is_playing():
                    if preset.lower() == "boost":
                        await player.set_filter(
                            wavelink.Filter(ctx.voice_client.filter, equalizer=wavelink.Equalizer.boost()))
                        await ctx.send(f"Equalizer set to **Boost**.")
                    elif preset.lower() == "flat":
                        await player.set_filter(
                            wavelink.Filter(ctx.voice_client.filter, equalizer=wavelink.Equalizer.flat()))
                        await ctx.send(f"Equalizer set to **Flat**.")
                    elif preset.lower() == "piano":
                        await player.set_filter(
                            wavelink.Filter(ctx.voice_client.filter, equalizer=wavelink.Equalizer.piano()))
                        await ctx.send(f"Equalizer set to **Piano**.")
                    elif preset.lower() == "metal":
                        await player.set_filter(
                            wavelink.Filter(ctx.voice_client.filter, equalizer=wavelink.Equalizer.metal()))
        ...
        if ctx.author.voice:
            ...
            if 0 <= num <= 100:
                ...
                return await ctx.send(
                    f"Volume inputted should be greater than or equal to 0 but less than or equal to 100.")
        else:
            return await ctx.send(f"{ctx.author.mention} You are not connected to a voice channel.")
